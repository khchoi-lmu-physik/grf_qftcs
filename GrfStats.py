...
Original file is located at
    https://colab.research.google.com/github/khchoi-physik/grf_cosmo_qftcs/blob/main/GrfStats.ipynb
"""
...
def plot_multiple_runs_multiprocessing( num_gpu, main_path, data_prefix, ylabel, total_steps, time_steps, xlabel= r'$\eta/\eta_0$' ):
    """
    Plots the results of multiple simulation runs across multiple GPUs on
    the same graph, including the mean and standard deviation of all runs.

    Args:
        num_gpu (int): Number of GPUs used in the simulations.
        main_path (str): The main directory path where the
    ...
    # Collect all relevant .npy files from each GPU's directory
    for gpu_id in range(num_gpu):
        npy_data = np.concatenate( (npy_data, glob.glob(main_path + f'/gpu{gpu_id}/{data_prefix}_*.npy') ) )
    ...
    for data_name in npy_data:
        ...
        # Set the line width and color
        linewidth = 2.5 if run_id == 1 else 1
        color = 'red' if run_id == 1 else 'green'
        alpha = 0.7 if run_id == 1 else 0.2
        zorder = 2 if run_id == 1 else 1
    ...
    # Compute the running mean and standard deviation across all simulation runs
    running_mean , running_std = multiple_run_std_mean(npy_data, total_steps, time_steps)
    ...
    plt.ylabel(ylabel, fontsize=18, color='black')
    plt.gca().yaxis.get_offset_text().set_fontsize(18)

    plt.tick_params(axis='both', colors='black', direction='out', length=6, width=2, labelsize=18) # Set default tick label size
    #ax.grid(True, color='grey', linestyle='--', linewidth=0.2, alpha=0.2) # Custom grid properties
